...
# ---------------------------------------------------------------------------
#
# UdhInfo/_parse_udh/_strip_udh now live in ingestion/udh.py - moved there
# once SS7 was found to need them too (see that module's docstring). Kept
# imported under their original names here since this module's own
# docstrings/tests refer to them that way.
...
def _decode_row(content_hex, dcs) -> dict:
    ...
    return {
        ...
        "udh_dest_port": udh.dest_port,  # Application Port Addressing IE
        # (0x04/0x05) - None unless this
        # message is port-addressed (WAP
        # Push/OTA/binary app traffic).
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    decoded = df.apply(
        lambda r: _decode_row(r.get("content"), r.get("dcs")),
        axis=1,
        result_type="expand",
    )
...
def map_to_canonical(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    missing_required = [
        c
        for c in ("record_id", "text_clean", "text_decode_failed")
    ...
    features = pd.DataFrame(
        {
            canonical: df[raw]
            for canonical, raw in FEATURE_MAP.items()
